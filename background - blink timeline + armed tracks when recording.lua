...
- if user switches to a different project tab while armed tracks are highlighted, color gets committed (any way to avoid this?)

Best solution for bugs is to associate the track GUID (not sufficient to know the track index alone—have to be able to follow tracks)
...
-- { {trackIdx, armed, colorIndex, trackMT}, ... }
allTracks = {} 
...
local function allTracksRefresh ()
  ...
  for i = 0, projTrackCount_current - 1 do
    ...
    table.insert( allTracks, { trackIdx = i, armed = 0 , colorIndex = reaper.GetTrackColor(currentTrackMT), trackMT = currentTrackMT, trackGUID = currentTrackGUID } )
...
local function allTracksPrint ()
  for idx, track in ipairs(allTracks) do 
    if track then 
      Msg("allTracks[" .. idx .. "] = { trackIdx = " .. track.trackIdx .. ", armed = " .. track.armed .. ", colorIndex = " .. track.colorIndex .. ", trackGUID = " .. track.trackGUID .. " }")
    else 
